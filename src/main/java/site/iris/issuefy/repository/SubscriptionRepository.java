...
public interface SubscriptionRepository extends CrudRepository<Subscription, Long> {
	...
	Optional<Page<Subscription>> findPageByUserId(@Param("userId") Long userId, Pageable pageable);

	@Query("SELECT s FROM Subscription s JOIN s.repository r JOIN r.org " +
		"WHERE s.user.id = :userId AND s.repoStarred = true")
	Optional<Page<Subscription>> findPageByUserIdAndRepoStarredTrue(@Param("userId") Long userId, Pageable pageable);
	...
	Optional<List<Subscription>> findByRepositoryId(Long repositoryId);
}
